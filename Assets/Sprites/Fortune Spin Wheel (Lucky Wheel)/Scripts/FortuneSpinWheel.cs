...
namespace JSG.FortuneSpinWheel
{
    public class FortuneSpinWheel : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                // Carrega o sprite salvo, se existir
                string spritePath = PlayerPrefs.GetString("RewardSprite_" + i, null);
                if (!string.IsNullOrEmpty(spritePath))
                {
                    Sprite savedSprite = Resources.Load<Sprite>(spritePath);
                    if (savedSprite != null)
                    {
                        m_RewardPictures[i].sprite = savedSprite;
                    }
                }
                else
                {
        ...
        // Método para salvar o sprite
        public void SaveSprite(int index, Sprite sprite)
        {
            string spritePath = AssetDatabase.GetAssetPath(sprite);
            PlayerPrefs.SetString("RewardSprite_" + index, spritePath);
            PlayerPrefs.Save();
        }
    }
}